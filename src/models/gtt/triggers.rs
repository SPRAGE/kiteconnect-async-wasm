use crate::models::common::{Exchange, GttStatus, OrderType, Product, TransactionType};
...
impl GTT {
    ...
    pub fn successful_order_ids(&self) -> Vec<&str> {
        self.orders
            .iter()
            .filter_map(|order| {
                order.result.as_ref().and_then(|result| {
