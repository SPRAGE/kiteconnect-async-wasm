...
use crate::models::common::{Exchange, Product, TransactionType, OrderType};
use super::triggers::{GTTCondition, GTTOrderParams, GTTCreateParams, GTTTriggerType};
...
impl GTTOrderBuilder {
    ...
    pub fn build(self) -> Result<GTTOrderParams, String> {
        Ok(GTTOrderParams {
            ...
            transaction_type: self.transaction_type.ok_or("Transaction type is required")?,
...
impl StopLossGTTBuilder {
    ...
    pub fn build_market(self) -> Result<GTTCreateParams, String> {
        let condition = GTTConditionBuilder::new()
            .exchange(self.exchange.ok_or("Exchange is required")?)
            .trading_symbol(self.trading_symbol.clone().ok_or("Trading symbol is required")?)
