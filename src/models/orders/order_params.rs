...
impl BracketOrderBuilder {
    ...
    pub fn build(self) -> Result<BracketOrderParams, String> {
        ...
        let squareoff = self.squareoff.ok_or("Square off value is required for bracket orders")?;
        let stoploss = self.stoploss.ok_or("Stoploss value is required for bracket orders")?;
