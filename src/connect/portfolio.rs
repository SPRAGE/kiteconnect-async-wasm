...
use crate::models::portfolio::{Holding, Position, ConversionRequest};
...
impl KiteConnect {
    ...
    pub async fn margins(&self, segment: Option<String>) -> Result<JsonValue> {
        if let Some(segment) = segment {
            let resp = self.send_request_with_rate_limiting_and_retry(
                ...
                None,
                None
            ).await.map_err(|e| anyhow::anyhow!("Get margins failed: {:?}", e))?;
            self.raise_or_return_json(resp).await
        } else {
            let resp = self.send_request_with_rate_limiting_and_retry(
                KiteEndpoint::Margins, 
                &[],
                None,
                None
            ).await.map_err(|e| anyhow::anyhow!("Get margins failed: {:?}", e))?;
    ...
    pub async fn profile(&self) -> Result<JsonValue> {
        let resp = self.send_request_with_rate_limiting_and_retry(
            ...
            None,
            None
        ).await.map_err(|e| anyhow::anyhow!("Get profile failed: {:?}", e))?;
        self.raise_or_return_json(resp).await
    }
    ...
    pub async fn holdings(&self) -> Result<JsonValue> {
        let resp = self.send_request_with_rate_limiting_and_retry(
            ...
            None,
            None
        ).await.map_err(|e| anyhow::anyhow!("Get holdings failed: {:?}", e))?;
        self.raise_or_return_json(resp).await
    }
    ...
    pub async fn positions(&self) -> Result<JsonValue> {
        let resp = self.send_request_with_rate_limiting_and_retry(
            KiteEndpoint::Positions, 
            &[],
            None,
            None
        ).await.map_err(|e| anyhow::anyhow!("Get positions failed: {:?}", e))?;
        self.raise_or_return_json(resp).await
    }
