...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Instrument: {} (RELIANCE)", daily_request.instrument_token);
    ...
    println!("  Instrument: {} (RELIANCE)", intraday_request.instrument_token);
    println!("  From: {}", intraday_request.from.format("%Y-%m-%d %H:%M:%S"));
    ...
    println!("  Instrument: {} (NIFTY Future)", futures_request.instrument_token);
    println!("  From: {}", futures_request.from.format("%Y-%m-%d %H:%M:%S"));
    ...
    for interval in intervals {
        println!("  {} -> String: '{}', Integer: {}", 
            format!("{:?}", interval), interval, interval.as_i8());
    }
    println!();
