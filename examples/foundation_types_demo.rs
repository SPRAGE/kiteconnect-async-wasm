...
fn test_response_types() {
    ...
    println!(
        "  ✓ Success Response: status={}, has_data={}",
        success_response.status,
        success_response.data.is_some()
    );

    // Create error response
    let error_response =
        KiteResponse::<Vec<String>>::error("Invalid session", Some("TokenException".to_string()));

    println!(
        "  ✓ Error Response: status={}, error_type={:?}",
