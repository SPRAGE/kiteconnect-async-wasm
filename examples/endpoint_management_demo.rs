...
async fn demonstrate_endpoint_system() {
    ...
    println!("  Order History: {}", KiteEndpoint::OrderHistory.build_path(&["order_123"]));
    println!("  Order Trades: {}", KiteEndpoint::OrderTrades.build_path(&["order_123", "trades"]));
    println!("  Historical Data: {}", KiteEndpoint::HistoricalData.build_path(&["738561", "day"]));
    println!();
    
    // Show endpoints by category
    println!("Endpoints by rate limit category:");
    for category in [RateLimitCategory::Quote, RateLimitCategory::Historical, 
                     RateLimitCategory::Orders, RateLimitCategory::Standard] {
        let endpoints = KiteEndpoint::by_rate_limit_category(category.clone());
        println!("  {:?} ({} req/sec): {} endpoints", 
            category, category.requests_per_second(), endpoints.len());
    }
    println!();
}
...
async fn demonstrate_rate_limiting_config() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Rate limiting enabled: {}", client_with_limits.is_rate_limiting_enabled());
    ...
    println!("  Rate limiting enabled: {}", client_no_limits.is_rate_limiting_enabled());
    
    // Show rate limit information
    println!("\nOfficial KiteConnect API Rate Limits:");
    println!("  Quote endpoints: {} request/second", RateLimitCategory::Quote.requests_per_second());
    println!("  Historical endpoints: {} requests/second", RateLimitCategory::Historical.requests_per_second());
    println!("  Order endpoints: {} requests/second", RateLimitCategory::Orders.requests_per_second());
    println!("  Standard endpoints: {} requests/second", RateLimitCategory::Standard.requests_per_second());
    println!();
...
async fn demonstrate_rate_limiting_in_action() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Second request - Can request immediately: {}", can_request_again);
    ...
    let can_request_different = client.can_request_immediately(&KiteEndpoint::Holdings).await;
    println!("  Different category (Holdings) - Can request immediately: {}", can_request_different);
    ...
    for i in 1..=3 {
        ...
        client.wait_for_request(&KiteEndpoint::Quote).await;
        println!("  Request {} completed in: {:?} (total: {:?})", 
            i, request_start.elapsed(), overall_start.elapsed());
    }
...
async fn demonstrate_rate_limiter_stats() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (category, category_stats) in &stats.categories {
        ...
        println!("    Request count: {}/{}", 
            ...
            category_stats.requests_per_second
        );
        println!("    Remaining capacity: {}", category_stats.remaining_capacity());
        ...
        if let Some(next) = category_stats.next_available {
            if next > Instant::now() {
                println!("    Next available: {:?}", next.duration_since(Instant::now()));
            } else {
    ...
    println!("  Rate limiting enabled: {}", client.is_rate_limiting_enabled());
...
mod tests {
    ...
    async fn test_rate_limiting() {
        ...
        assert!(client.can_request_immediately(&KiteEndpoint::Holdings).await);
    }
